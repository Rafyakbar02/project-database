...
const format = (amount) => {
  if (amount > 1000000000000) {
    ...
    return "Rp " + (amount / 1000000000000).toFixed(1) + " miliar";
  } else if (amount > 1000000) {
    return "Rp " + (amount / 1000000000000).toFixed(1) + " juta";
  } else return formattterIDR.format(amount);
};
  ...
  return (
    <div className={"z-10"} key={id}>
      <div
        ...
        onClick={() => setOpen((p) => !p)}
      >
        <div className={"col-span-1 flex justify-center items-center"}>{i}</div>
          ...
          >
            ...
            <div className={"col-span-5"}>
              <Segment
                ...
                content={format(totalInvestment)}
              />
              <Segment title="Signing Date" content={signingDate} />
            </div>
            <div className={"col-span-6 lg:col-span-4"}>
              <Segment title="Total Exposure" content={format(totalExposure)} />
              <Segment
            ...
            >
              <Link href={`/projects/${id}`}>
                <Button type="button" label="View Project" size="sm" />
              </Link>
            </div>
          </motion.div>
        )}
